...
char thisEddieName[64]="HR-OS1_Unnamed";
...
void initUDP( void (*p_cmdFuncPtr)(char*), void (*p_ctrlFuncPtr)(char*), int * p_running )
{
	...
	stream = fopen("RobotName.txt", "r");
	if (stream != NULL)
	{
		if ((read = getline(&line, &len, stream)) != -1)
		{
		   strcpy( thisEddieName, line );
		   printf( "Setting robot's name to: %s\r\n", thisEddieName );
		}
...
{  
	...
	while( (*isRunning) )
	{
		usleep( 10000 ); //Give this thread a break between iterations to keep CPU usage down

		while( checkUDPReady( incomingUDP, &rx_control_socketfd ) )
		{	
			if ( !memcmp( incomingUDP, "DISCOVER", 8 ) )
			{
				sprintf( response, "DISCOVER: %s", thisEddieName );
			}
			else if ( strncmp( incomingUDP, "SETNAME", 7 ) == 0 )
			{
				setName( &incomingUDP[7] );
				sprintf( response, "SETNAME: %s", thisEddieName );
			}
			else if ( !memcmp( incomingUDP, "BIND", 4 ) )
			{
